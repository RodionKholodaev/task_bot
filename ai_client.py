...
async def classify_task(description: str) -> dict: 
    ...
    2) "type": "chat" — если в тексте НЕТ задач (размышления, эмоции, бессмысленный текст, вопросы не про задачи, или попытка отредактировать задачу).
    ...
    ФОРМАТ ОТВЕТА, ЕСЛИ ЗАДАЧ НЕТ:

    {
    "type": "chat",
    "message": "дружелюбный и понятный текстовый ответ пользователю"
    }

    Сообщение должно:
    - объяснять, что задачи не обнаружены
    - предлагать помощь с постановкой задач
    - быть дружелюбным
    - юмор допустим, но умеренно
    ...
    {
    "type": "chat",
    "message": "Звучит философски 🙂 Но задач я тут не вижу. Напиши, что нужно сделать — я помогу всё разложить."
    }

    --------------------------------------------------

    ПРИМЕР 4 (БЕССМЫСЛЕННЫЙ ТЕКСТ):
    ввод:
    ...
    {
    "type": "chat",
    "message": "Кажется, тут закралась опечатка 🙂 Напиши задачу, и я помогу её спланировать."
    }

    --------------------------------------------------

    ПРИМЕР 5 (пользователь хотел изменить задачу что создал)
    ввод:
    сегодня 2026-10-12 15:03, завтра

    ответ:
    {
    "type": "chat",
    "message": "Кажется, вы хотели отредактировать задачу. Свайпните ее влево и напишите что нужно исправить."
    }

    ПРИМЕР 6 (пользователь хотел изменить задачу, что создал)
    ввод:
    сегодня 2026-05-03 11:16, ой нет нужно сделать это завтра
    ...
    - Никогда не смешивай задачи и текстовые ответы.
...
async def edit_task(information: dict, date_and_time: str) -> dict:
    ...
    1) "type": "tasks" — если в тексте есть одна или несколько задач.
    2) "type": "chat" — если в тексте НЕТ задач (размышления, эмоции, бессмысленный текст, вопросы не про задачи).
    ...
    Если "type": "chat" (то есть если пользователь написал что-то что не относится к задаче)
    {{
    "type": "chat",
    "message": "дружелюбный и понятный текстовый ответ пользователю"
    }}
    ...
    ЕСЛИ НАЧАЛЬНИК УКАЗАЛ КАКОЙ-ТО БРЕД, например:
    ...
    {{
    "type": "chat",
    "message": "Видимо тут есть опечатки, если тебе нужно отредактировать задачу, напиши конкретнее"
    }}

    ОТВЕЧЧАЙ ВЕЖЛЕВО И С ЮМОРОМ!
