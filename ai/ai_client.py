...
async def parse_text(description: str, user_description: str) -> dict: 
    ...
    Достать задачи из текста и записать их максимально понятно БЕЗ ПОТЕРИ ИНФОРМАЦИИ.
    ...
    5. Если ты записываешь задачу запиши ее максимально понятно. Нельзя терять ни капли информации, что отправил пользоваетль.
    6. Если есть возможность запиши задачу самым красмвым образом
    ...
    НЕЛЬЗЯ ТЕРЯТЬ НИ КАПЛИ ИНФОРМАЦИИ
    НЕЛЬЗЯ ТЕРЯТЬ НИ КАПЛИ ИНФОРМАЦИИ
    
    ВСЮ ИНФОРМАЦИЮ О САМОЙ ЗАДАЧЕ(БЕЗ ВРЕМЕНИ И ДАТЫ), ПОЛУЧЕННУЮ ОТ ПОЛЬЗОВАТЕЛЯ ЗАПИСЫВАЙ В task
    ...
    ПРИМЕР 4:
    ввод:
    сегодня Monday, 2025-10-10 10:17,
    ...
    ПРИМЕР 5 (пользователь просит напомнить через какое-то количество дней):
    ввод:
    сегодня Friday, 2026-03-20 10:17, у меня запись к врачу на 2 апреля напомни через 2 дня
    Вот информация о днях недели: ...
    ответ:
    {
    "type": "tasks",
    ...
    ПРИМЕР 6 (пользователь написал покупку но указал время):
    ввод:
    ...
    Ты будешь получать информацию о днях недели от пользователя. Важно учитывать ее чтобы правильно сопоставлять дни недели и даты.
    --------------------------------------------------
