...
async def _get_user_notify_time(user_id: int) -> Optional[dict]:
    """Получить notify_time и utc_offset пользователя"""
    user = await UserRepository.get_user_settings(user_id)
...
async def load_all_jobs_from_db():
    ...
    for user in users:
        ...
        tasks = await TaskRepository.get_tasks_with_reminder(user.user_id) #type: ignore
        for task in tasks:
