...
async def start(message: Message, s: AsyncSession, command: CommandObject, bot: Bot):
    ...
    msg = await message.answer(
        ...
        "*Что я умею:*\n\n"
        "🤖 *Понимаю текст и голос* — просто напиши или запиши голосовое: «Купить хлеб в 18:00» или «Созвон в пятницу в 19, напомни за час», и я создам задачу.\n\n"
        "🔔 *Напоминаю о делах* — пришлю список задач на день и напомню о любой из них в удобное время.\n\n"
        "🛒 *Веду список покупок* — сохраняю покупки, группирую по категориям и показываю в удобном виде.\n\n"
        "⏳ *Сортирую по времени* — помогу найти быстрые пятиминутки или важные и сложные дела.\n\n"
        "📅 *Планирую* — покажу задачи на сегодня, завтра, неделю или весь список сразу.\n\n"
        "✏️ *Редактирую ответом* — чтобы изменить задачу или покупку, просто свайпни её сообщение влево и напиши правки!\n\n"
        "Настрой свой часовой пояс в профиле, перед началом работы бота",
        reply_markup=new_main_keyboard(),
        parse_mode="Markdown"
    )
    # await message.answer(
    #     "📌 *Краткая инструкция*\n\n"
    #     "• Пиши или диктуй голосом задачи и покупки:\n"
    #     "→ «Созвон в пятницу в 19, напомни за час», «Купить хлеб, молоко, яйца».\n\n"
    #     "• Для изменения задачи/покупки ответь на сообщение и укажи, что поправить.\n\n"
    #     "• Если указана дата, я напомню о ней (время общего напоминания настраивается в профиле).\n\n"
    #     "• Все покупки доступны в разделе: 🛒 покупки.\n\n"
    #     "• Задачи делятся по срокам (сегодня/неделя) и по длительности (от 5 мин до сложных).\n\n"
    #     "• Заполни описание в профиле, чтобы я точнее оценивал время на твои задачи.\n\n"
    #     "⚙️ Не забудь настроить часовой пояс в профиле!",
    #     parse_mode="Markdown"
    # )
    ...
    # кажется не стоит вводить оплату пока
...
async def settings(message: Message):
    ...
    await message.answer(
        "Укажи время ежедневного напоминания и часовой пояс:\nUTC_OFFSET HH:MM\n\nПример:\n+3 09:00\n(UTC+3 в 9 утра)"
    )
...
async def show_statistics(message: Message, s: AsyncSession):
    ...
    # Формируем покупки
    shopping_lines = [f"  🛒 {categories_map.get(cat,'Разное')}: <b>{cnt}</b>" for cat, cnt in stats['shopping']]
